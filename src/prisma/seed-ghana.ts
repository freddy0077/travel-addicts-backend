import { PrismaClient, UserRole, TourStatus, DifficultyLevel, PostStatus, BookingStatus, PaymentStatus, PaymentMethod } from '@prisma/client';
import bcrypt from 'bcryptjs';
import { ghanaDestinations } from './data/destinations';
import { ghanaDestinationsExtended } from './data/destinations-extended';
import { ghanaTours } from './data/tours';
import { tourItineraries } from './data/itineraries';
...
async function main() {
  ...
  try {
    ...
    const allDestinations = [...ghanaDestinations, ...ghanaDestinationsExtended];
    ...
    for (const tour of ghanaTours) {
      ...
      const tourItinerary = tourItineraries.find(itinerary => itinerary.tourSlug === tour.slug);
      if (tourItinerary) {
    ...
    // Content manager
    const hashedPasswordContent = await bcrypt.hash('admin123', 12);
    await prisma.user.upsert({
    ...
    for (const customer of customers) {
      const hashedPasswordCustomer = await bcrypt.hash('admin123', 12);
      await prisma.user.upsert({
