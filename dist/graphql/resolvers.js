...
const resolvers = {
    Query: {
        ...
        destinationBySlug: async (_, { slug }) => {
            return await prisma.destination.findUnique({
                where: { slug },
                include: {
                    country: true,
        ...
        searchDestinations: async (_, args) => {
            const { query, continent, country, type, season, minPrice, maxPrice, duration, minRating, features, limit = 20, offset = 0 } = args;
            // Build where clause
            const where = {};
        ...
        // Tours
        tours: async (_, { destinationId, featured }, context) => {
            const where = {};
            if (destinationId) {
        ...
        tourPricing: async (_, { tourId }) => {
            return await prisma.tourPricing.findMany({
                where: { tourId },
                orderBy: { season: 'asc' }
            });
        },
        searchTours: async (_, args) => {
            const { query, continent, country, destination, category, minPrice, maxPrice, duration, minRating, features, season, startDate, endDate, adults, children, limit = 12, offset = 0 } = args;
            ...
            if (features && features.length > 0) {
                tours = tours.filter(tour => {
                    ...
                    return features.some(feature => tour.inclusions.some(inclusion => inclusion.toLowerCase().includes(feature.toLowerCase())));
                });
            }
            // Season filter (based on destination season or tour-specific season)
            if (season) {
                tours = tours.filter(tour => tour.destination && tour.destination.season &&
        ...
        // Blog Posts
        blogPosts: async (_, { status, featured }) => {
            const where = {};
            if (status)
                where.status = status;
            if (featured !== undefined)
        ...
        // Bookings
        bookings: async (parent, { status }, context) => {
            if (!context.user) {
                throw new Error('Not authenticated');
            }
        ...
        analytics: async (_, __, context) => {
            ...
            try {
                ...
                const popularDestinations = await Promise.all(popularDestinationsData.map(async (item) => {
                ...
                const topPerformingTours = await Promise.all(topPerformingToursData.map(async (item) => {
                    const tour = await prisma.tour.findUnique({
                        where: { id: item.tourId }
                    });
                    return {
        ...
        userActivity: async (_, __, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Admin access required');
            }
            try {
        ...
        contentStats: async (_, __, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Admin access required');
            }
            try {
        ...
        financialSummary: async (_, __, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Admin access required');
            }
            try {
        ...
        performanceMetrics: async (_, __, context) => {
            if (!context.user || !['SUPER_ADMIN'].includes(context.user.role)) {
                throw new Error('Super admin access required');
            }
            try {
        ...
        allUsers: async (_, { limit = 50, offset = 0 }, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Admin access required');
            }
            try {
        ...
        userStats: async (_, { userId }, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Admin access required');
            }
            try {
        ...
        recentContent: async (_, __, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Admin access required');
            }
            try {
        ...
        contentByStatus: async (_, { status }, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Admin access required');
            }
            try {
        ...
        mediaFiles: async (_, { limit = 50, offset = 0 }, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Admin access required');
            }
            try {
        ...
        // Gallery Images
        galleryImages: async (_, { filters, limit = 50, offset = 0 }, context) => {
            try {
                const where = {};
                if (filters) {
                    if (filters.category) {
        ...
        galleryImage: async (_, { id }, context) => {
            try {
    ...
    Mutation: {
        ...
        // Destinations
        createDestination: async (_, { input }, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
        ...
        updateDestination: async (_, { id, input }, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Insufficient permissions');
            }
        ...
        deleteDestination: async (_, { id }, context) => {
        ...
        // Tours
        createTour: async (_, { input }, context) => {
            // if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
            //   throw new Error('Insufficient permissions');
            // }
        ...
        updateTour: async (_, { id, input }, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Insufficient permissions');
            }
            const updateData = { ...input, status: input.status || 'PUBLISHED' };
            // Convert price to pesewas if provided
            if (input.priceFrom) {
                updateData.priceFrom = Math.round(input.priceFrom);
            }
        ...
        createTourItinerary: async (_, { input }, context) => {
            if (!context.user) {
                throw new Error('Authentication required');
            }
        ...
        updateTourItinerary: async (_, { id, input }, context) => {
            if (!context.user) {
                throw new Error('Authentication required');
            }
        ...
        deleteTourItinerary: async (_, { id }, context) => {
            if (!context.user) {
                throw new Error('Authentication required');
            }
            // Verify the itinerary exists
        ...
        updateBlogPost: async (_, { id, input }, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER', 'MARKETING'].includes(context.user.role)) {
                throw new Error('Insufficient permissions');
            }
            const updateData = { ...input };
        ...
        updateBookingStatus: async (parent, { id, status }, context) => {
            ...
            return await prisma.booking.update({
                where: { id },
                data: { status: status }, // Cast to avoid enum type issues
                include: {
                    tour: {
                        include: {
                            destination: {
        ...
        createBooking: async (_, { input }) => {
            ...
            catch (error) { // Properly type the error
        ...
        paystackInitialize: async (_, { input }) => {
            try {
                console.log('🔄 Initializing Paystack payment:', input);
                const response = await paystack_1.paystackService.initializeTransaction({
                    email: input.email,
        ...
        paystackVerify: async (_, { reference }) => {
            try {
                console.log('🔍 Verifying Paystack payment:', reference);
                const response = await paystack_1.paystackService.verifyTransaction(reference);
                return {
                    success: response.status,
                    message: response.message,
                    data: response.status ? {
        ...
        // Media
        generatePresignedUploadUrl: async (_, { input }, context) => {
            // if (!context.user) {
            //   throw new Error('Authentication required');
            // }
            const { filename, contentType, folder } = input;
            // Validate file type
            if (!mediaService_1.mediaService.isValidImageType(contentType)) {
                throw new Error('Invalid file type. Only images are allowed.');
            }
            try {
                // Generate folder path if not provided
                const folderPath = folder || mediaService_1.mediaService.getFolderPath('general');
                const result = await mediaService_1.mediaService.generatePresignedUploadUrl(filename, contentType, folderPath);
                return result;
            }
            catch (error) {
        ...
        createMediaFile: async (_, { input }, context) => {
            // if (!context.user) {
            //   throw new Error('Authentication required');
            // }
            try {
        ...
        deleteMediaFile: async (_, { id }, context) => {
            // if (!context.user) {
            //   throw new Error('Authentication required');
            // }
            try {
                ...
                // Delete from S3
                if (mediaFile.key) {
                    await mediaService_1.mediaService.deleteFile(mediaFile.key);
                }
        ...
        serverUploadFile: async (_, { input }, context) => {
            // if (!context.user) {
            //   throw new Error('Authentication required');
            // }
            ...
            try {
                ...
                // Validate file size
                if (!mediaService_1.mediaService.isValidFileSize(buffer.length)) {
                ...
                const uploadResult = await mediaService_1.mediaService.uploadFile(buffer, filename, contentType, folderPath);
        ...
        createTourPricing: async (_, { input }, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Insufficient permissions');
            }
        ...
        updateTourPricing: async (_, { id, input }, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Insufficient permissions');
            }
            const updateData = { ...input };
        ...
        deleteTourPricing: async (_, { id }, context) => {
            if (!context.user || !['SUPER_ADMIN', 'CONTENT_MANAGER'].includes(context.user.role)) {
                throw new Error('Insufficient permissions');
            }
        ...
        createManualBooking: async (_, { input }, context) => {
            if (!context.user) {
                throw new Error('Authentication required');
            }
            // Check if user has permission (admin roles)
        ...
        recordPayment: async (_, { input }, context) => {
            if (!context.user) {
                throw new Error('Authentication required');
            }
            // Check if user has permission (admin roles)
        ...
        // Gallery Images
        createGalleryImage: async (_, { input }, context) => {
            if (!context.user) {
                throw new Error('Authentication required');
            }
            if (!['SUPER_ADMIN', 'CONTENT_MANAGER', 'MARKETING'].includes(context.user.role)) {
        ...
        updateGalleryImage: async (_, { id, input }, context) => {
            if (!context.user) {
                throw new Error('Authentication required');
            }
            if (!['SUPER_ADMIN', 'CONTENT_MANAGER', 'MARKETING'].includes(context.user.role)) {
    ...
    Tour: {
        ...
        rating: (parent) => {
            ...
            const totalRating = parent.reviews.reduce((sum, review) => sum + review.rating, 0);
